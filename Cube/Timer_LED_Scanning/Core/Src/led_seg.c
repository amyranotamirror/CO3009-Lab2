...
int led_buffer[NUM_LED] = {1, 2, 3, 4};
void init_led_seg(){
...
void update7SEG(int index){
	...
	case 0:
		...
		disable_led_seg(2);
		disable_led_seg(3);
		display7SEG(led_buffer[0], GPIOB);
		break;
	case 1:
		disable_led_seg(0);
		enable_led_seg(1);
		disable_led_seg(2);
		disable_led_seg(3);
		display7SEG(led_buffer[1], GPIOB);
		break;
	case 2:
		disable_led_seg(0);
		disable_led_seg(1);
		enable_led_seg(2);
		disable_led_seg(3);
		display7SEG(led_buffer[2], GPIOB);
		break;
	case 3:
		disable_led_seg(0);
		disable_led_seg(1);
		disable_led_seg(2);
		enable_led_seg(3);
		display7SEG(led_buffer[3], GPIOB);
		break;
	default:
		disable_led_seg(0);
		disable_led_seg(0);
